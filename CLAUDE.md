...
> 本文件是 VocabMaster 仓库级共享 SSOT，记录项目结构、验证和改动后要求。`AGENTS.md` 是 companion entry，只记录当前 agent 入口的工具差异；共享规则不要复制到两个文件。
...
- **源码** 全部放在 `src/`：`src/main.go`、`src/cmd/`、`src/internal/`、`src/data/`、`src/tools/`
- **build 产物** `build/`（Makefile `go build -o build/<binary>`）
- **根目录** 只放元数据和入口文件：`go.mod`、`go.sum`、`Makefile`、`README.md`、`CLAUDE.md`、`AGENTS.md`、`ref/`
- **数据资产** `src/data/`（词库 JSON）— 入 git，运行时只读
...
4. **数据资产改动**（`src/data/` 词库 JSON）按数据更新流程处理，不入 changelog 主线（视情况落 review）
